...
select * 
from ( 
	select 'cashbook_report_allow_multiple_fund_selection' as name, '0' as value, 'Cashbook Report: Allow Multiple Fund Selection' as description, 'checkbox' as datatype, 'TC' as category
	union
	select 'liquidate_remittance_as_of_date' as name, '1' as value, 'Liquidate Remittances as of Date' as description, 'checkbox' as datatype, 'TC' as category 
	union 
	select 'cashreceipt_reprint_requires_approval' as name, 'false' as value, 'CashReceipt Reprinting Requires Approval' as description, 'checkbox' as datatype, 'TC' as category
	union 
	select 'cashreceipt_void_requires_approval' as name, 'true' as value, 'CashReceipt Void Requires Approval' as description, 'checkbox' as datatype, 'TC' as category
	union 
	select 'deposit_collection_by_bank_account' as name, '0' as value, 'Deposit collection by bank account instead of by fund' as description, 'checkbox' as datatype, 'TC' as category 
)t0 
where (select count(*) from sys_var where name = t0.name) = 0 
; 
