...
select 
    ...
    pt.tag AS item_tag 
from itemaccount ia 
    inner join itemaccount p onia.parentid = p.objid 
    inner join itemaccount_tag pt onp.objid = pt.acctid 
    inner join sys_org o onia.org_objid = o.objid 
...
alter table cashreceipt_rpt_share_forposting_repost 
  add CONSTRAINT cashreceipt_rpt_share_forposting_repost_ibfk_1 
  FOREIGN KEY (receiptid) REFERENCES cashreceipt (objid)
go
alter table cashreceipt_rpt_share_forposting_repost 
  add CONSTRAINT cashreceipt_rpt_share_forposting_repost_ibfk_2 
